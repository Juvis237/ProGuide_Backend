    ...
    >
        ...
        <div class="modal-inner modal-sm">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">{{__('edits.update_status')}}</h5>
            </div>
            <div class="modal-body ">
                 <p class="text-center"> {{__('edits.other_deletes')}}</p>

                 <p class="text-center"> {{__('tables.title')}} : <span>{{$blog->title}}</span></p>
            ...
            <div class="modal-footer">
                ...
                <button type="button" wire:click="publish('{{$blog->status==="published"?"draft":"published"}}')" class="btn btn-primary" wire:loading.attribute = 'disabled'>
                    ...
                    {{$blog->status==="published"? __('edits.unpublish'): __('edits.publish')}}
                </button>
            </div>
        </div>
        @endif
