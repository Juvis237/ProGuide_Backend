    ...
    >
        <div class="modal-inner">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">{{__('edits.delete_user')}} </h5>
            </div>
            <div class="modal-body ">
                 <p class="text-center"> {{__('edits.delete_user_question')}} ?</p>
                @if(isset($user))
                 <p class="text-center"> {{__('common.name')}} : <span>{{$user->name}}</span></p>
                @endif
            </div>
